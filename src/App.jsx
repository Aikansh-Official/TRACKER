...
export default function App() {
  ...
  return <main className={dark ? 'app today-app dark' : 'app today-app'}>
    <section className="content today-content">
      ...
      <section className="task-layout"><div className="task-list"><div className="list-title"><span>IN PROGRESS <b>{scoredTasks.filter(t => !t.done).length}</b></span><span>{completed} finished · {tasks.filter(task => task.skipped).length} resting</span></div>{tasks.length === 0 && <div className="inline-empty"><b>Your day is still unwritten.</b><span>Add one meaningful intention, then give it your attention.</span></div>}{tasks.map(task => <article className={`task-card ${task.done ? 'done' : ''} ${task.skipped ? 'skipped' : ''} ${task.kind === 'temporary' ? 'temporary' : ''}`} key={task.id}><button className={`check ${task.done ? 'checked' : ''}`} onClick={() => toggle(task.id)} aria-label={`Mark ${task.title} ${task.done ? 'not done' : 'done'}`}>{task.done && '✓'}</button><div className={`task-icon ${task.color}`}><Icon>{task.icon}</Icon></div><div className="task-info"><h3>{task.title}{task.kind === 'temporary' && <span className="tag">TEMPORARY</span>}</h3><p>{task.note}</p>{task.kind === 'quantity' && <div className="quantity-progress"><i style={{width: `${task.value / task.target * 100}%`}}/><span>{task.value}/{task.target}</span></div>}</div>{task.kind === 'quantity' && !task.skipped && <div className="count-controls"><button className="count-button subtract" onClick={() => decrease(task.id)} disabled={task.value <= 0}>− 1</button><button className="count-button" onClick={() => increase(task.id)} disabled={task.value >= task.target}>+ 1</button></div>}</article>)}</div>
...
function CalendarPage({ data, tasks, routines, token, loading, error, dark, onNavigate, onChanged, notify }) {
